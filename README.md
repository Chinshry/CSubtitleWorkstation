...
传统字幕压制工作流依赖 BAT 脚本 + 命令行，存在以下痛点：

- **参数繁琐**：ffmpeg 命令行参数众多，易出错，难以复用
- **反馈缺失**：BAT 窗口闪现即关，看不到实时进度和日志
- **跨平台困难**：BAT 仅 Windows，macOS/Linux 用户无法使用
...
- 🖼️ **LOGO 可视化编辑** — 在视频抽帧上拖放 LOGO，支持四角缩放，按分辨率桶记忆布局（720p/1080p/4K × 横竖屏）
