...
- 🎬 **拖拽导入**：视频、字幕拖入窗口即可开始；支持单视频处理，也支持视频 + 字幕同时导入。
- 📊 **视频信息解析**：自动读取分辨率、帧率、编码器、时长、CFR/VFR 等关键信息。
- 🎨 **编码参数可视化**：质量值、码率、编码器集中配置；质量值可留空，留空时不生成 `-crf` / `-cq` / `-qp`。
- 🎛️ **视频处理**：支持反交错、旋转、镜像、缩放、帧率和视频码率调整，直接并入同一次压制。
- 👁️ **命令预览与日志**：开始前可展开完整 ffmpeg 命令；压制中显示进度、速度、fps、码率、输出大小和 stdout/stderr。
- ⏹️ **任务取消**：压制过程中可取消，优雅退出 ffmpeg，已写入片段尽量保留可播放。

### 字幕与 LOGO

- 🔤 **普通字幕压制**：使用 ffmpeg libass 渲染 ASS / SSA / SRT / VTT / SUB 字幕。
- 🧠 **特效字幕识别**（Windows）：自动扫描 VSFilterMod 扩展标签，命中后提示并可切换 AVS 压制。
- 🧩 **AVS 特效压制**（Windows）：通过 AviSynth+ / VSFilterMod 处理复杂 ASS 特效、矢量绘制和 img 标签。
- 🔍 **字幕检查面板**：集中提示图片缺失、字体未安装、样式缺失、ASS 色彩矩阵和特效标签风险。
- 🖼️ **LOGO 可视化编辑**：在视频抽帧上拖放、缩放 LOGO；按分辨率和屏幕方向保存布局，可配置字幕与 LOGO 层级。

### 工具页

- 📝 **文字处理**
  - 🧾 **CC 字幕整理**：拆分 Web CC 说话人标签，整理花字行 / 听轴行，支持 SRT 转 ASS。
  - 🔁 **繁简转换**：实时预览、差异高亮、同步滚动，自定义词库优先匹配。
  - ✅ **字幕校对**：检查「的 / 地 / 得」疑似误用，支持自定义词库、正则和试匹配，只提示不自动替换。
- 🔄 **格式转换**
  - 🔄 **字幕格式转换**：ASS / SSA / SRT / VTT 互转，单字幕拖入可直接进入工具。
  - 📦 **视频转 MP4**：常见视频容器重新封装为 MP4，默认 stream copy 不重新编码。
- 🎞️ **媒体处理**
  - 🎞️ **TS 分片合并**：按顺序合并 TS / M2TS / MTS 分片并输出 MP4，不重新编码。
  - 🎧 **合并音视频**：保留视频画面，合并单独音频来源输出 MP4。
  - 🖼️ **添加封面**：给 MP4 写入 JPG / PNG 封面，原音视频流原样复制。

### 预设

- 🧰 **编码预设管理**：内置 x264 / x265 / NVENC / AMF / VideoToolbox 预设，可新增、编辑、导入导出。
- 📝 **输出文件名模板**：支持 `{video_name}` / `{resolution}` / `{encoder}` / `{crf}` / `{date:YYYYMMDD}` 等变量。

### 环境与更新

- 🔔 **应用更新检查**：启动时静默检查或在设置页手动检查，新版本通过 Toast 提醒。
- 🍎 **macOS 兼容自检**：检测 ffmpeg 是否包含 `subtitles` / `ass` filter，缺失时提示安装 `ffmpeg-full`。
...
<table>
  ...
  <tr>
    ...
    <td width="50%" align="center">
      ...
      <br/><sub><b>工具 / 设置</b> · 字幕校对、繁简转换、CC 字幕整理、字幕格式转换、封装转换、自定义词库与 ffmpeg / AviSynth+ 自检</sub>
    </td>
  </tr>
</table>
...
<details>
<summary><strong>自定义词库怎么用？</strong></summary>

两个文字工具都支持自定义词库，但用途不同：

- **字幕校对**：进入「工具」→「字幕校对」。可以粘贴文本，也可以拖入 TXT / ASS / SSA / SRT / VTT / SUB 文件。应用会标出「的 / 地 / 得」疑似误用和词库命中的专有名词问题；点击问题可定位原文，点击「采纳」才会替换，不会自动改动原文。校对词库用于统一人名、团名、节目名、术语等写法，每条规则由「标准写法」和「匹配规则」组成，匹配规则支持正则，`%1` 可引用第 1 个捕获组。
- **繁简转换**：进入「工具」→「繁简转换」。左侧粘贴文本后会实时转换，右侧高亮差异并同步滚动。自定义词库每行一条，支持 `原词 = 目标词`、`原词 -> 目标词`、`原词 => 目标词` 或 Tab 分隔；词库优先级最高，会先保护命中的词再执行基础繁简转换，例如 `朴乾旭 = 朴乾旭` 可以避免被转换成 `朴干旭`。

两个词库都会保存到本地配置，下次打开自动恢复。词库窗口里的「试匹配」可以输入一小段文本，实时查看当前规则会命中什么、建议替换成什么。拖入 TXT 做繁简转换时，应用只读取并预览结果；确认后点击「导出结果」，目标文件已存在时会先确认是否覆盖。
