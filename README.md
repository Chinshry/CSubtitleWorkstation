...
- 🎨 **可视化参数** — 质量值、码率、编码器（x264/x265/NVENC/AMF/VideoToolbox）一目了然；质量值可留空，留空时不生成 `-crf` / `-cq` / `-qp` 参数。
- 🎛️ **视频处理** — 压制页内置旋转、镜像、分辨率、帧率、视频码率处理；旋转和镜像独立配置，支持 180° 旋转、横向/竖向镜像、自定义缩放，复用同一套命令预览与压制流程。
- 🖼️ **LOGO 可视化编辑 + 布局保存** — 在视频抽帧上拖放 LOGO，支持四角缩放、画布快捷键缩放、手动输入缩放比例；按分辨率和屏幕方向保存布局，下次打开自动恢复。
- 🎞️ **反交错处理** — yadif 可选开关，处理交错素材。
- 🔤 **特效字幕压制**（仅 Windows）— 使用 AviSynth+ 脚本引擎处理复杂特效字幕（如矢量绘制、img 标签等），相比 ffmpeg libass 支持更完善。
- 🧠 **特效标签自动识别**（仅 Windows）— 选定字幕后自动扫描 VSFilterMod 扩展标签，命中即自动勾选「AVS 压制」并提示具体标签。
- 🔍 **字幕检查面板** — 集中展示图片路径缺失、字体未安装、样式缺失、ASS 色彩矩阵和 VSFilterMod 特效标签等风险。
- 👁️ **命令预览** — 开始压制前预览完整 ffmpeg 参数，支持折叠和复制。
- 📈 **实时进度与日志** — 展示进度、当前时间、输出大小、速度、fps、码率和 ffmpeg stdout/stderr。
- ⏹️ **取消任务** — 压制过程中随时取消，进程优雅退出，已编码片段保留可播放。

### 预设

- 🧰 **编码预设管理** — 内置 `x264 平衡` / `x265 体积优先` / `NVENC 快速` / `AMF 快速` / `Apple 快速` 等多套预设，可新增、编辑、导入导出，主页直接切换。
- 📝 **输出文件名模板** — 支持 `{video_name}` / `{resolution}` / `{encoder}` / `{crf}` / `{date:YYYYMMDD}` 等变量，可保存多套模板并设为默认。

### 工具

- 🧰 **工具页** — 侧边栏「工具」集中放置文本与媒体辅助工具，当前内置字幕校对、繁简转换、CC 字幕整理与封装转换。
- ✅ **字幕校对** — 支持粘贴文本或拖入 TXT / ASS / SSA / SRT / VTT / SUB 文件，检查「的 / 地 / 得」疑似误用，并可通过自定义词库统一人名、团名和专有名词写法；只提示，不自动替换。
- 🔎 **校对词库试匹配** — 自定义词库支持正则匹配、捕获组替换和本地记忆，编辑时可输入测试文本即时查看命中片段和建议替换结果。
- 🔁 **繁体 / 简体转换** — 支持实时预览、差异高亮、左右同步滚动、自定义词库优先匹配和本地记忆。
- 📄 **TXT 拖拽转换** — 拖入 TXT 后先读取并预览转换结果，可补充自定义词库后再手动导出；目标文件已存在时会确认是否覆盖。
- 🧾 **CC 字幕整理** — 将 Web CC 说话人标签拆分为花字行与听轴行，支持台词清理、样式参考、自定义替换词典和 SRT 转 ASS 输出。
- 🎞️ **封装转换** — 支持单文件转 MP4、TS / M2TS / MTS 分片合并转 MP4、给 MP4 添加封面、合并视频与外部音频，默认只复制音视频流，不重新编码；TS 输入会自动处理 ADTS AAC 到 MP4 的 bitstream filter。
...
| **文本 / 字幕工具** | TXT, ASS, SSA, SRT, VTT, SUB |
...
<table>
  ...
  <tr>
    ...
    <td width="50%" align="center">
      ...
      <br/><sub><b>工具 / 设置</b> · 字幕校对、繁简转换、CC 字幕整理、封装转换、自定义词库与 ffmpeg / AviSynth+ 自检</sub>
    </td>
  </tr>
</table>
...
<details>
<summary><strong>字幕校对和自定义词库怎么用？</strong></summary>

进入侧边栏「工具」→「字幕校对」。可以直接粘贴文本，也可以拖入 TXT / ASS / SSA / SRT / VTT / SUB 文件。应用会标出疑似问题，点击问题可定位原文，点击「采纳」才会替换；不会自动改动原文。

当前内置校对包括「的 / 地 / 得」疑似误用提示。自定义词库用于统一专有名词写法，例如人名、团名、节目名、术语等。每条规则由「标准写法」和「匹配规则」组成，匹配规则支持正则，`%1` 可引用第 1 个捕获组。词库窗口里的「试匹配」可以输入一小段文本，实时查看当前规则会命中什么、建议替换成什么。

校对词库会保存到本地配置，下次打开自动恢复。

</details>

<details>
<summary><strong>繁简转换工具怎么使用自定义词库？</strong></summary>

进入侧边栏「工具」→「繁简转换」。左侧粘贴文本后会实时转换，右侧结果会高亮差异，并与左侧同步滚动。

自定义词库每行一条，支持 `原词 = 目标词`、`原词 -> 目标词`、`原词 => 目标词` 或 Tab 分隔。自定义词库优先级最高，会先保护命中的词再执行基础繁简转换，例如 `朴乾旭 = 朴乾旭` 可以避免被转换成 `朴干旭`。词库会保存到本地配置，下次打开自动恢复。

拖入 TXT 文件时，应用只会读取并预览转换结果，不会立刻写文件。确认结果后点击「导出结果」，输出到原文件同目录；如果目标文件已存在，会先确认是否覆盖。
...
应用会在系统用户目录下保存配置、窗口状态、字幕校对词库、繁简转换自定义词库、WebView 缓存、压制临时字幕、LOGO 预览帧和 AVS 临时脚本。完整路径和清理建议见 [用户数据与缓存](docs/CACHE_AND_DATA.md)。
