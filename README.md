...
- **Apple Silicon (M1 / M2 / M3 / M4 等)**：下载 `CSubtitleWorkstation_x.y.z_aarch64.dmg`
- **Intel**：下载 `CSubtitleWorkstation_x.y.z_x64.dmg`

不确定自己机器是哪种？菜单 → 关于本机 → 看"芯片"字段。
...
AVS 兼容模式依赖 AviSynth+，仅 Windows 可用。macOS 版本会自动隐藏 AVS 相关选项，统一使用 ffmpeg libass 字幕渲染。
...
详见 [TODO.md](./TODO.md)。Windows NSIS 安装包与 macOS DMG 通过 GitHub Actions 自动构建发布；Tauri Updater 公私钥目前未启用。可视化 LOGO 叠加已完成（含多分辨率桶记忆与 rotation 视频修正）。
